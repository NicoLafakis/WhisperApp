...
def test_installer_iss_security_and_integrity():
    ...
    assert 'Source: "dist\\{#MyAppName}\\*"' in content, (
        "installer.iss must package the complete onedir payload"
    )
    assert "SignedUninstaller=yes" in content
    assert "SignTool=WhisperAppRelease" in content
...
def test_dev_certificate_install_is_explicit_opt_in():
    """Normal tests inspect policy without creating certs or touching user stores."""
    script = (ROOT / "scripts" / "Create-DevCert.ps1").read_text(encoding="utf-8")
    assert "InstallTrust" in script
    assert "TrustedPeople" in script
    assert "TrustedPublisher" in script
    assert "-Path Cert:\\CurrentUser\\My" in script
